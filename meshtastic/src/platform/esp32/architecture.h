...
#elif defined(CROWPANEL_DHE04005D)
// Reuse the upstream CROWPANEL enum (97) so the mobile app shows
// "CROWPANEL" in the node card instead of the generic "PRIVATE_HW".
// There's no DHE04005D-specific enum because the protobuf only carries
// the enum number — the human-readable string is derived from the enum
// name on the client side. Adding a new enum would require an upstream
// protobuf bump + mobile-app release, so we piggy-back on the existing
// CrowPanel identity (this is in fact a CrowPanel board, just a model
// that isn't individually enumerated in the official list).
#define HW_VENDOR meshtastic_HardwareModel_CROWPANEL
