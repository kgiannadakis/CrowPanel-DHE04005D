...
// On the P4 build mcui's UI task and the Arduino loopTask share core 1 (mcui
// pinned there to avoid an SMP PSRAM heap race). Long LVGL frames can starve
// the loopTask of CPU; bump the WDT timeout so transient stalls don't reboot.
#define APP_WATCHDOG_SECS 300
#else
#define APP_WATCHDOG_SECS 90
#endif

#if defined(CONFIG_IDF_TARGET_ESP32C6) || defined(CONFIG_IDF_TARGET_ESP32P4)
    // IDF 5.x watchdog API takes a config struct (C6 branch already on IDF 5;
    // P4 is on pioarduino's IDF 5.5).
    ...
    if (res == ESP_ERR_INVALID_STATE) {
        // Arduino-esp32's app_main pre-initialises TWDT on IDF 5.x.
        // Reconfigure (raise our timeout) instead of asserting.
