...
// Defined in src/crowpanel_backlight.cpp. The mcui UI task blocks on the FB
// gate just before esp_lcd_new_rgb_panel(); we release it once the early
// WiFi + MQTT socket bring-up below has finished on the still-clean heap.
...
#if defined(ARCH_ESP32P4) && defined(CROWPANEL_DHE04005D)
    // On the P4 RGB build, PSRAM is reserved for explicit users such as
    // esp_lcd framebuffers. Letting default malloc/new spill into PSRAM after
    // the RGB driver is up trips IDF TLSF asserts during ordinary file saves.
...
// Do this after service.init (because that clears error_code)
...
#endif
    bool earlyNetworkInitDone = false;

#if defined(ARCH_ESP32P4) && defined(CROWPANEL_DHE04005D) && !defined(ARCH_PORTDUINO)
#if HAS_WIFI
    initWifi();
    if (config.network.wifi_enabled && config.network.wifi_ssid[0]) {
        // Associate + DHCP now. The mcui UI task is blocked on the FB gate
        // (just before esp_lcd_new_rgb_panel(), which permanently corrupts
        // the PSRAM TLSF heap), so these lwIP/ESP-Hosted allocations land
        // on the still-clean heap.
        (void)wifiConnectNowBlocking(15000);
    }
#endif
#if !MESHTASTIC_EXCLUDE_MQTT
    mqttInit();
    ...
    // UI task proceed to esp_lcd_new_rgb_panel(). Always called (even with
    // WiFi/MQTT off) so the display can never deadlock on the gate.
    crowpanel_p4_fb_gate_release();
#endif
...
#if !MESHTASTIC_EXCLUDE_MQTT
    // On ESP-Hosted boards, bring network drivers up before the MQTT thread
    // starts attempting direct broker connections.
    if (!earlyNetworkInitDone) {
        mqttInit();
    }
#endif
