...
void PacketHistory::insert(const PacketRecord &r)
{
    ...
    if (tu->rxTimeMsec != 0) {
        LOG_DEBUG("Packet History - insert: Reusing slot aged %.3fs TRACE %s", OldtrxTimeMsec / 1000.,
                  (tu->id == r.id && tu->sender == r.sender) ? "MATCHED PACKET" : "OLDEST SLOT");
    } else {
        LOG_DEBUG("Packet History - insert: Using new slot @uptime %.3fs TRACE NEW", millis() / 1000.);
    }
#endif
