...
template <typename T> bool SX126xInterface<T>::isChannelActive()
{
    ...
    if (result == RADIOLIB_ERR_NONE) {
        ...
        while ((millis() - start) < timeoutMs) {
            ...
            if (irq & (RADIOLIB_SX126X_IRQ_CAD_DONE | RADIOLIB_SX126X_IRQ_CAD_DETECTED)) {
        ...
        if (result == RADIOLIB_ERR_NONE) {
            LOG_WARN("SX126X CAD timed out, treating channel as free");
            lora.clearIrqFlags(RADIOLIB_SX126X_IRQ_ALL);
            return false;
        }
    }
...
template <typename T> void SX126xInterface<T>::resetAGC()
{
    ...
    // Quiesce RX/TX and disable DIO1 IRQ callbacks before direct SPI writes.
    // This avoids concurrent radio SPI transactions while AGC calibration runs.
    setStandby();
    if (sendingPacket != NULL || (isReceiving && isActivelyReceiving()))
        return;
