 ...
 * |   1   |   1   |                 Yes                 |              Yes           |
 */
// Drop unused bus drivers from the binary entirely. Otherwise
// esp_panel_host_i2c.cpp gets linked in and aborts boot at i2c_driver_install
// against arduino-esp32 3.x's already-claimed i2c-NG driver:
 ...
 * |   1   |   1   |                 Yes                 |              Yes           |
 */
// 0 = drop the source for all LCDs except the one we actually use
// (EK79007). Saves a substantial chunk of flash (each panel driver is a
// dedicated .c file with init sequences and registers).
 ...
 * |   1   |   1   |                 Yes                 |              Yes           |
 */
// 0 = drop the source for all touch controllers except GT911.
 ...
 * Set to `1` to enable, `0` to disable.
 */
// We drive LCD backlight ourselves from display.cpp (LEDC on IO31), not
// through ESP_Panel's backlight subsystem. Disabling _USE_ALL + all four
// backlight drivers strips that whole code path from the binary.
