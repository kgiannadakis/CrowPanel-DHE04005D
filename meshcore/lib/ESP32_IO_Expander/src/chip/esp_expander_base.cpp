...
bool Base::init(void)
{
    ...
    // The original code calls i2c_param_config + i2c_driver_install (the
    // legacy IDF i2c.c API). Even though this branch is never reached on
    // the DHE04209D (we don't use any IO expander, USE_EXPANDER=0), the
    // mere reference forces the linker to pull libdriver.a(i2c.c.obj),
    // whose `.text.startup` constructor `check_i2c_driver_conflict()` then
    // aborts at boot because the new i2c-NG driver is also linked
    // (arduino-esp32 3.x's Wire). Returning false here without touching
    // legacy-I2C symbols stops the link from grabbing i2c.c.obj.
    if (!isHostSkipInit()) {
        ESP_UTILS_LOGE("IO expander I2C init not supported on this build "
                       "(legacy I2C driver disabled). Set HOST_SKIP_INIT and "
                       "create the i2c_master_bus yourself if you need an expander.");
        return false;
    }
...
bool Base::del(void)
{
    ...
    if (isOverState(State::INIT) && !isHostSkipInit()) {
        ESP_UTILS_LOGW("IO expander legacy I2C host cleanup skipped in this build");
    }
