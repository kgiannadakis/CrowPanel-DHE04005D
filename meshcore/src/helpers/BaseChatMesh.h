...
class BaseChatMesh : public mesh::Mesh {
  ...
  uint8_t _txt_header_bytes = TXT_HEADER_BYTES_1;  // tx format: 1-byte (legacy), 2-byte, or 3-byte
...
public:
  ...
  void setTextHeaderBytes(uint8_t bytes) {
    if (bytes == TXT_HEADER_BYTES_2 || bytes == TXT_HEADER_BYTES_3) _txt_header_bytes = bytes;
    else _txt_header_bytes = TXT_HEADER_BYTES_1;
  }
  uint8_t getTextHeaderBytes() const { return _txt_header_bytes; }
