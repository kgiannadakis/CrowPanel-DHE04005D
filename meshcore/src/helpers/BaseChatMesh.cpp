...
void BaseChatMesh::onPeerDataRecv(mesh::Packet* packet, uint8_t type, int sender_idx, const uint8_t* secret, uint8_t* data, size_t len) {
  ...
  if (type == PAYLOAD_TYPE_TXT_MSG && len > 5) {
    ...
    const uint8_t hdr0 = data[4];
    const bool has_ext_hdr = (hdr0 & TXT_FLAG_EXT_HDR) != 0;
    uint8_t hdr_len = has_ext_hdr ? TXT_HEADER_BYTES_2 : TXT_HEADER_BYTES_1;
    if (has_ext_hdr && len > 5) {
      const uint8_t ext = data[5] & 0x03;  // 0=>2-byte, 1=>3-byte
      if (ext == 1) hdr_len = TXT_HEADER_BYTES_3;
    }
    if (len <= (size_t)(4 + hdr_len)) {
      MESH_DEBUG_PRINTLN("onPeerDataRecv: dropping short txt payload len=%d hdr=%d", (uint32_t)len, (uint32_t)hdr_len);
      return;
    }
    const uint8_t* txt = &data[4 + hdr_len];
    uint8_t flags = (hdr0 >> 2) & 0x1F;   // message attempt number + msg type bits
    ...
    if (flags == TXT_TYPE_PLAIN) {
      ...
      onMessageRecv(from, packet, timestamp, (const char *)txt);  // let UI know

      int text_len = strlen((char *)txt);
      uint8_t ack_hash[6];    // calc truncated hash of the message timestamp + text + sender pub_key, to prove to sender that we got it
      mesh::Utils::sha256(ack_hash, 4, data, (size_t)(4 + hdr_len) + text_len, from.id.pub_key, PUB_KEY_SIZE);
      // NEW: append (potential) extended attempt byte (to make packethash unique)
      ack_hash[4] = data[(4 + hdr_len) + text_len + 1];
      ...
      onCommandDataRecv(from, packet, timestamp, (const char *)txt);  // let UI know
      ...
      if (len <= (size_t)(8 + hdr_len)) {
        MESH_DEBUG_PRINTLN("onPeerDataRecv: dropping short signed txt payload len=%d hdr=%d", (uint32_t)len, (uint32_t)hdr_len);
        return;
      }
      ...
      from.lastmod = getRTCClock()->getCurrentTime(); // update last heard time
      onSignedMessageRecv(from, packet, timestamp, &data[4 + hdr_len], (const char *) &data[8 + hdr_len]);  // let UI know

      uint32_t ack_hash;    // calc truncated hash of the message timestamp + text + OUR pub_key, to prove to sender that we got it
      mesh::Utils::sha256((uint8_t *) &ack_hash, 4, data, (size_t)(8 + hdr_len) + strlen((char *)&data[8 + hdr_len]), self_id.pub_key, PUB_KEY_SIZE);
...
void BaseChatMesh::onGroupDataRecv(mesh::Packet* packet, uint8_t type, const mesh::GroupChannel& channel, uint8_t* data, size_t len) {
  if (type == PAYLOAD_TYPE_GRP_TXT) {
    ...
    if (len <= (size_t)(4 + hdr_len)) {
      MESH_DEBUG_PRINTLN("onGroupDataRecv: dropping short group text payload len=%d hdr=%d", (uint32_t)len, (uint32_t)hdr_len);
      return;
    }
    uint8_t txt_type = (hdr0 >> 2) & 0x1F;
    ...
    onChannelMessageRecv(channel, packet, timestamp, (const char *) &data[4 + hdr_len]);  // let UI know
...
mesh::Packet* BaseChatMesh::composeMsgPacket(const ContactInfo& recipient, uint32_t timestamp, uint8_t attempt, const char *text, uint32_t& expected_ack) {
  ...
  uint8_t hdr_len = TXT_HEADER_BYTES_1;
  if (_txt_header_bytes == TXT_HEADER_BYTES_2 || _txt_header_bytes == TXT_HEADER_BYTES_3) hdr_len = _txt_header_bytes;
  const uint8_t hdr0 = (attempt & 3) | ((hdr_len >= TXT_HEADER_BYTES_2) ? TXT_FLAG_EXT_HDR : 0);

  uint8_t temp[7+MAX_TEXT_LEN+1];
  memcpy(temp, &timestamp, 4);   // mostly an extra blob to help make packet_hash unique
  temp[4] = hdr0;
  if (hdr_len >= TXT_HEADER_BYTES_2) temp[5] = (hdr_len == TXT_HEADER_BYTES_3) ? 1 : 0;  // extension flags
  if (hdr_len == TXT_HEADER_BYTES_3) temp[6] = 0;  // reserved
  memcpy(&temp[4 + hdr_len], text, text_len + 1);

  // calc expected ACK reply
  mesh::Utils::sha256((uint8_t *)&expected_ack, 4, temp, (size_t)(4 + hdr_len) + text_len, self_id.pub_key, PUB_KEY_SIZE);

  int len = (4 + hdr_len) + text_len;
  if (attempt > 3) {
...
int  BaseChatMesh::sendCommandData(const ContactInfo& recipient, uint32_t timestamp, uint8_t attempt, const char* text, uint32_t& est_timeout) {
  ...
  uint8_t hdr_len = TXT_HEADER_BYTES_1;
  if (_txt_header_bytes == TXT_HEADER_BYTES_2 || _txt_header_bytes == TXT_HEADER_BYTES_3) hdr_len = _txt_header_bytes;
  const uint8_t hdr0 = (attempt & 3) | (TXT_TYPE_CLI_DATA << 2) | ((hdr_len >= TXT_HEADER_BYTES_2) ? TXT_FLAG_EXT_HDR : 0);
  ...
  auto pkt = createDatagram(PAYLOAD_TYPE_TXT_MSG, recipient.id, recipient.getSharedSecret(self_id), temp, (4 + hdr_len) + text_len);
  if (pkt == NULL) return MSG_SEND_FAILED;
...
bool BaseChatMesh::sendGroupMessage(uint32_t timestamp, mesh::GroupChannel& channel, const char* sender_name, const char* text, int text_len) {
  uint8_t hdr_len = TXT_HEADER_BYTES_1;
  if (_txt_header_bytes == TXT_HEADER_BYTES_2 || _txt_header_bytes == TXT_HEADER_BYTES_3) hdr_len = _txt_header_bytes;
  uint8_t temp[7+MAX_TEXT_LEN+32];
  memcpy(temp, &timestamp, 4);   // mostly an extra blob to help make packet_hash unique
  temp[4] = (hdr_len >= TXT_HEADER_BYTES_2) ? TXT_FLAG_EXT_HDR : 0;  // TXT_TYPE_PLAIN + optional ext flag
  if (hdr_len >= TXT_HEADER_BYTES_2) temp[5] = (hdr_len == TXT_HEADER_BYTES_3) ? 1 : 0;  // extension flags
  if (hdr_len == TXT_HEADER_BYTES_3) temp[6] = 0;  // reserved

  sprintf((char *) &temp[4 + hdr_len], "%s: ", sender_name);  // <sender>: <msg>
  char *ep = strchr((char *) &temp[4 + hdr_len], 0);
  int prefix_len = ep - (char *) &temp[4 + hdr_len];

  if (text_len + prefix_len > MAX_TEXT_LEN) text_len = MAX_TEXT_LEN - prefix_len;
  memcpy(ep, text, text_len);
  ep[text_len] = 0;  // null terminator

  auto pkt = createGroupDatagram(PAYLOAD_TYPE_GRP_TXT, channel, temp, (4 + hdr_len) + prefix_len + text_len);
  if (pkt) {
