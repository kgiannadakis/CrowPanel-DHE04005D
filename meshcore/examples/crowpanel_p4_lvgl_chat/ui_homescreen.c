...
void ui_homescreen_screen_init(void) {
    ...
    lv_obj_set_style_radius(ui_mutebutton, 4, 0);
    ...
    // Chat panel ends 8 px ABOVE the text input top, not at the bottom of
    // the screen. Previously the panel filled SCR_H - CHATPANEL_START_Y
    // all the way down to the tab bar; the pad_bottom style kept content
    // clear of the text input WHEN scrolled to max, but at any other
    // scroll position bubbles could render under the text input and peek
    // through the narrow strip between its bottom and the tab bar top.
    // Constraining the panel's physical size eliminates that leakage
    // regardless of scroll position. When the keyboard shows, the text
    // input moves to KB_TA_Y and the keyboard covers everything below —
