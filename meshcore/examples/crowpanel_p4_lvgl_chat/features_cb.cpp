...
#include <WiFi.h>   // WiFi.localIP() for the Dashboard-toggle IP hint
...
extern "C" void features_register_callbacks() {
    ...
    reg(ui_wifisaveddd,      cb_wifi_saved_changed, LV_EVENT_VALUE_CHANGED);
    ui_apply_wifi_state();
...
extern "C" void features_update_status_labels() {
    // Fast gate: if the features screen isn't actually on-screen, the
    // labels don't need updating — leaving the widgets untouched saves
    // LVGL an invalidate+re-measure cycle. Status changes that happen
    // while the user is elsewhere get picked up the next time they
    // open the Web Apps tab.
    if (!ui_featuresscreen || lv_disp_get_scr_act(NULL) != ui_featuresscreen) {
        return;
    }

    // Cache the last-applied state so we don't keep re-setting identical
    // text on every dirty pulse (WiFi state churn during reconnect can
    // fire this path many times in a row).
    static bool     s_last_tg_on        = false;
    static bool     s_last_wd_on        = false;
    static bool     s_last_ota_updating = false;
    static bool     s_last_ota_busy     = false;
    static uint8_t  s_last_ota_pct      = 0xFF;
    static String   s_last_tg_txt;
    static String   s_last_wd_txt;
    static String   s_last_ota_txt;
    static bool     s_first = true;

    // Telegram toggle color
    if (ui_tg_toggle && (s_first || s_last_tg_on != g_tgbridge_enabled)) {
        lv_obj_set_style_bg_color(ui_tg_toggle,
            lv_color_hex(g_tgbridge_enabled ? TH_GREEN : TH_SURFACE2), 0);
    }
    s_last_tg_on = g_tgbridge_enabled;

    if (ui_tg_status_lbl) {
        const char* t = tgbridge_status_text();
        if (s_first || s_last_tg_txt != t) {
            lv_label_set_text(ui_tg_status_lbl, t);
            s_last_tg_txt = t;
        }
    }

    // Web dashboard toggle color
    if (ui_wd_toggle && (s_first || s_last_wd_on != g_webdash_enabled)) {
        lv_obj_set_style_bg_color(ui_wd_toggle,
            lv_color_hex(g_webdash_enabled ? TH_GREEN : TH_SURFACE2), 0);
    }
    s_last_wd_on = g_webdash_enabled;

    if (ui_wd_status_lbl) {
        const char* t = webdash_status_text();
        if (s_first || s_last_wd_txt != t) {
            lv_label_set_text(ui_wd_status_lbl, t);
            s_last_wd_txt = t;
        }
    }

    // Dashboard toggle button label: carries the "Visit <IP> for GPS
    // and Dashboard" hint that used to live on the WiFi toggle. Shows
    // the IP when WiFi is connected so the user knows where to point
    // their browser — regardless of whether the dashboard is currently
    // enabled (they can see it before enabling, too).
    if (ui_wd_toggle_lbl) {
        static String s_last_wd_lbl;
        char buf[128];
        if (g_wifi_connected) {
            snprintf(buf, sizeof(buf),
                     LV_SYMBOL_EYE_OPEN " Web Dashboard\nVisit: %s for GPS and Dashboard",
                     WiFi.localIP().toString().c_str());
        } else {
            snprintf(buf, sizeof(buf), LV_SYMBOL_EYE_OPEN " Web Dashboard");
        }
        if (s_first || s_last_wd_lbl != buf) {
            lv_label_set_text(ui_wd_toggle_lbl, buf);
            s_last_wd_lbl = buf;
        }
    }
    ...
    if (ui_ota_status_lbl) {
        const char* t = ota_status_text();
        if (s_first || s_last_ota_txt != t) {
            lv_label_set_text(ui_ota_status_lbl, t);
            s_last_ota_txt = t;
        }
    }
    if (ui_ota_progress_bar) {
        bool updating = ota_is_updating();
        uint8_t pct   = ota_progress_percent();
        if (updating != s_last_ota_updating) {
            if (updating) lv_obj_clear_flag(ui_ota_progress_bar, LV_OBJ_FLAG_HIDDEN);
            else          lv_obj_add_flag  (ui_ota_progress_bar, LV_OBJ_FLAG_HIDDEN);
        }
        if (updating && pct != s_last_ota_pct) {
            lv_bar_set_value(ui_ota_progress_bar, pct, LV_ANIM_OFF);
        }
        s_last_ota_updating = updating;
        s_last_ota_pct      = pct;
    }
    if (ui_ota_check_btn) {
        bool busy = ota_is_checking() || ota_is_updating();
        if (s_first || busy != s_last_ota_busy) {
            if (busy) lv_obj_add_state  (ui_ota_check_btn, LV_STATE_DISABLED);
            else      lv_obj_clear_state(ui_ota_check_btn, LV_STATE_DISABLED);
        }
        s_last_ota_busy = busy;
    }
    s_first = false;
}
