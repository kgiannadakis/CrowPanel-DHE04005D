...
lv_obj_t * ui_repeaterpathbutton   = NULL;
lv_obj_t * ui_Label_path           = NULL;
...
// CLI mode
lv_obj_t * ui_clibutton            = NULL;
lv_obj_t * ui_Label_cli            = NULL;
lv_obj_t * ui_repeatercliinput     = NULL;
...
void ui_repeaterscreen_screen_init(void) {
    ...
    // CLI input — same row position as the password field, hidden
    // until the CLI button toggles into terminal mode. Created BEFORE
    // ui_repeaterpassword so the password is on top in z-order; the
    // CLI handler swaps which one is hidden when entering/exiting CLI.
    ui_repeatercliinput = lv_textarea_create(body);
    lv_obj_set_size(ui_repeatercliinput, lv_pct(100), 40);
    lv_textarea_set_one_line(ui_repeatercliinput, true);
    lv_textarea_set_max_length(ui_repeatercliinput, 120);
    lv_textarea_set_placeholder_text(ui_repeatercliinput, "$ command...");
    style_ta(ui_repeatercliinput);
    lv_obj_add_flag(ui_repeatercliinput, LV_OBJ_FLAG_HIDDEN);
    ...
    lv_obj_set_style_radius(ui_repeaterloginbutton, 4, 0);
    ...
    // Originally mid_row / mon_wrap / btn_col were all LV_SIZE_CONTENT and
    // mon_wrap used flex_grow=1 (portrait). That combination triggered an
    // LVGL 8.3 layout-recursion freeze on action-button press: the press
    // state-style on a btn_col child re-measured the column → mid_row had
    // to re-flex → mon_wrap (content-sized + flex_grow) re-measured → its
    // child label (lv_pct(100) width, content height, long-wrap) re-measured
    // → mon_wrap re-measured again → infinite. CPU stayed alive but
    // lvgl_task spun forever inside lv_timer_handler, which is exactly the
    // "screen frozen but mesh still works" symptom we observed.
    //
    // Fix: pin every dimension on this row. Label gets a fixed pixel width
    // (parent width minus padding) so wrap is deterministic and doesn't
    // feed back into mon_wrap's size.

    // 5 buttons (Status/Advert/Neighbours/CLI/Reboot) + 4×6 gap
    // = 5*52 + 24 = 284. Round up to 300 for visual breathing room.
    const int16_t MID_ROW_H = 300;
    int16_t mon_w;
    int16_t btn_col_w;
    if (SCR_H < SCR_W) {
      // Landscape: monitor 400 px, buttons fill the remainder.
      mon_w     = 400;
      btn_col_w = SCR_W - 32 - mon_w - 6;   // 32 = body horiz pad on both sides; 6 = pad_column
      if (btn_col_w < 200) btn_col_w = 200;
    } else {
      // Portrait: button column fixed at BTN_COL_W, monitor fills the rest.
      btn_col_w = BTN_COL_W;
      mon_w     = SCR_W - 32 - btn_col_w - 6;
      if (mon_w < 200) mon_w = 200;
    }

    lv_obj_t * mid_row = lv_obj_create(body);
    lv_obj_set_size(mid_row, lv_pct(100), MID_ROW_H);
    ...
    // ── Status monitor (left, fixed size, scrolls long content) ────────────

    lv_obj_t * mon_wrap = lv_obj_create(mid_row);
    lv_obj_set_size(mon_wrap, mon_w, MID_ROW_H);
    ...
    lv_obj_set_style_radius(mon_wrap, 4, 0);
    ...
    // Fixed pixel width — break the parent↔child size feedback loop that
    // was triggering the LVGL flex-recursion freeze. mon_w minus the
    // left+right pad of mon_wrap (8+8=16) and a couple px for safety.
    lv_obj_set_width(ui_repeatermonitor, mon_w - 18);
    ...
    // ── Action buttons column (right, fixed size) ──────────────────────────

    lv_obj_t * btn_col = lv_obj_create(mid_row);
    lv_obj_set_size(btn_col, btn_col_w, MID_ROW_H);
    ...
    lv_obj_set_style_pad_row(btn_col, 4, 0);
    lv_obj_set_flex_flow(btn_col, LV_FLEX_FLOW_COLUMN);
    lv_obj_clear_flag(btn_col, LV_OBJ_FLAG_SCROLLABLE);

    {
      // Smaller button height now that we host 5 buttons in the
      // (300-px tall) column. 5 × 52 + 4 × 6 = 284 — fits with margin.
      lv_coord_t bcw = (SCR_H < SCR_W) ? lv_pct(96) : (BTN_COL_W - 4);
      int16_t    bch = 44;
      ui_statusbutton         = make_btn_v(btn_col, &ui_Label19,    bcw, bch, TH_ACCENT);
      ui_repeaterpathbutton   = make_btn_v(btn_col, &ui_Label_path, bcw, bch, TH_ACCENT);
