...
#if defined(ESP32)
  // P4 port: LittleFS replaces SPIFFS (see persistence.cpp + main.cpp notes —
  // SPIFFS format on a 10 MB partition blows the watchdog).
  #include <LittleFS.h>
...
#if defined(ESP32)
// Cross-core guard for MeshCore mutable state (contacts/channels/routes).
// LVGL callbacks run on the dedicated LVGL task while g_mesh->loop() runs
// on the Arduino task; code that iterates contacts while loop() mutates them
// must take this mutex.
extern SemaphoreHandle_t g_mesh_mutex;
#endif
...
// Key of the chat currently being viewed (ct_<hex> for PM, ch_<idx> for
// channel). Empty while not in chat mode. Used by translate.cpp to
// decide whether a completed translation should live-update the
// chatpanel, and by load_chat_from_file() to set the active key.
extern char      g_current_chat_key[20];
...
extern bool   g_ui_prefs_save_dirty;
extern uint32_t g_ui_prefs_save_ms;
// One-shot suppression flag: set by load_chat_from_file() when it
// positions the view at the "NEW MESSAGES" divider, consumed by
// enter_chat_mode() so it doesn't stamp a scroll-to-bottom over it.
extern bool   g_suppress_next_scroll_bottom;
// Divider to scroll into view once LVGL finishes laying out fresh
// chat bubbles. Set by load_chat_from_file, cleared by the main
// loop after scrolling.
extern lv_obj_t* g_pending_scroll_to_divider;
// Map viewport (lat/lon/zoom/filter) debounce flags, mirroring the
// g_ui_prefs_save_* pattern. Flush handled by the main-loop
// housekeeping block.
extern bool   g_map_viewport_save_dirty;
extern uint32_t g_map_viewport_save_ms;
// True while ui_chatpanel's children are the home list (the product of
// the last build_homescreen_list() call). Flipped false by chat_add /
// chat_clear when they populate the panel with chat content.
extern bool   g_homescreen_list_is_current;
...
// 0..100 volume percent. Scales the I2S sine amplitude in utils.cpp
// (beep_task / play_tone_blocking). 0 mutes without disabling the
// speaker; users can still feel the device ack via haptic alternatives.
extern uint8_t g_speaker_volume;
// Index into the notification-sound bank defined in utils.cpp
// (notification_sound_name / notification_sound_play). 0 = rising
// chime (the default). Persisted to NVS key "ui/snd_idx".
extern uint8_t g_notification_sound_idx;
...
// Time (NTP-driven — no hardware RTC on this variant)
...
// g_selected_repeater is a SHORT-LIVED pointer — only valid for the
// duration of a single callback after refresh_selected_repeater() (or
// equivalent lookupContactByPubKey) has resolved it. Never store
// across yield points: BaseChatMesh::removeContact() compacts the
// underlying contacts[] array and any pointer above the deleted slot
// is dangling after that call.
extern ContactInfo* g_selected_repeater;
// Snapshot of repeater contacts for the dropdown. Holds copied data
// (pubkey + name + path-len) instead of pointers, so it stays valid
// across contact-array shuffles. Look up the live ContactInfo by
// pubkey at use time.
struct RepeaterListEntry {
  uint8_t pub_key[32];
  char    name[32];
  uint8_t out_path_len;
};
extern RepeaterListEntry g_repeater_list[MAX_DD_ENTRIES];
