...
lv_obj_t* ui_wifisaveddd         = NULL;  // saved-networks dropdown
...
static void style_dd(lv_obj_t* dd) {
    ...
    lv_obj_set_style_border_opa(dd, LV_OPA_COVER, LV_PART_MAIN);
...
void ui_featuresscreen_screen_init(void) {
    ...
    // Saved-networks dropdown. Lists up to 5 stored networks (from
    // NVS); selecting one switches the active WiFi slot and
    // reconnects — no need to re-scan / re-enter a password when
    // moving between familiar networks. Initially hidden if no
    // slots exist; features_cb populates it on screen show.
    ui_wifisaveddd = lv_dropdown_create(form);
    lv_obj_set_width(ui_wifisaveddd, lv_pct(100));
    lv_dropdown_set_options(ui_wifisaveddd, "(no saved networks)");
    style_dd(ui_wifisaveddd);
    ...
    make_section_hdr(form, "TRANSLATION (available only when online)");
    {
        lv_obj_t* hint = lv_label_create(form);
    ...
    // ══════════════════════════════════════════════════════════
    // WEB DASHBOARD  (master switch for all cloud/server features)
    // ══════════════════════════════════════════════════════════

    make_section_hdr(form, "WEB DASHBOARD");

    ui_wd_toggle = make_action_btn(form, &ui_wd_toggle_lbl, lv_pct(100), 72);
    // cb_wd_toggle + ui_apply_webdash_state wire the click + label in
    // features_cb.cpp. Label text is set there from the current
    // g_webdash_enabled state so it reads e.g. "🔆 Web Dashboard\nOFF"
    // or "🔆 Web Dashboard\nON  http://<ip>".

    ui_wd_status_lbl = lv_label_create(form);
    lv_label_set_text(ui_wd_status_lbl, "");
    lv_obj_set_style_text_color(ui_wd_status_lbl, lv_color_hex(TH_TEXT3), 0);
    lv_obj_set_style_text_font(ui_wd_status_lbl, &lv_font_montserrat_12, 0);
    lv_obj_set_width(ui_wd_status_lbl, lv_pct(100));
    ...
    // (Maps was a button here in Phase 1 — promoted to a dedicated
    // tab in the bottom tab bar, so the Web Apps form no longer
    // needs an entry for it.)

    // ── TAB BAR ─────────────────────────────────────────────

    ui_tabbar_create(ui_featuresscreen, 3);   // Web Apps is tab index 3 now (Maps inserted at 2)
