...
// Dropdown change — user picked a different saved network.
// Switches the active slot + reconnects (via wifi_slot_activate).
void cb_wifi_saved_changed(lv_event_t*);
void ui_apply_wifi_state();
void wifi_ui_update_status();
// Rebuilds the saved-networks dropdown options from the current
// multi-slot NVS state. Called from wifi_ui_update_status() so any
// place that refreshes WiFi status also refreshes the dropdown.
