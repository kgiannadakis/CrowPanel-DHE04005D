...
static void tab_maps_cb(lv_event_t * e) {
    (void)e;
    _ui_screen_change(&ui_mapscreen, LV_SCR_LOAD_ANIM_NONE, 0, 0,
...
static lv_obj_t * make_tab_btn(lv_obj_t * bar, const char * icon,
    ...
    // 5 tabs now (Chats / Repeaters / Maps / Web Apps / Settings).
    // Width divisor follows the tab count in ui_tabbar_create.
    lv_obj_set_size(btn, SCR_W / 5, TAB_H);
...
void ui_tabbar_create(lv_obj_t * parent, int active_idx) {
    ...
    // Tab order (and index values passed by screen init sites):
    //   0 Chats     1 Repeaters   2 Maps   3 Web Apps   4 Settings
    ...
    make_tab_btn(bar, LV_SYMBOL_GPS,       "Maps",      active_idx == 2, tab_maps_cb);
    make_tab_btn(bar, LV_SYMBOL_WIFI,      "Web Apps",  active_idx == 3, tab_webapps_cb);
    make_tab_btn(bar, LV_SYMBOL_SETTINGS,  "Settings",  active_idx == 4, tab_settings_cb);
}